...
const RresetPassword = () => {
	...
	const [isVisibleResetPasswordInputs, setIsVisibleResetPasswordInputs] = useState<boolean>(false);
	const [newPassword, setNewPassword] = useState<string>("");
	const [newPasswordConfirmed, setNewPasswordConfirmed] = useState<string>("");

	const send2FAemailHandle = async (event: React.KeyboardEvent<HTMLInputElement> | React.MouseEvent<HTMLButtonElement>) => {
		...
		let response = await send2FAemail(email, securityCodeGenerated);
		if (response === "1") {
			setIsVisibleResetPasswordInputs(true);
		}
	};

	const resetPassword = async (event: React.KeyboardEvent<HTMLInputElement> | React.MouseEvent<HTMLButtonElement>) => {
		if ("key" in event && event.key === "Enter" || event.type === "click") {
			event.preventDefault();
		} else {
			return;
		}
		if (securityCodeGenerated !== securityCodeFromInput) {
			InfoPopup("Invalid code");
			return;
		} else if (newPassword !== newPasswordConfirmed || newPassword === "") {
			...
			await fetch(`/api/updatePassword/${email}`, {
				...
				.then(async data => {
					if (data.message === "Password updated successfully") {
						...
						InfoPopup("Error resetting password");
					}
				});
		} catch (error) {
			console.error(error);
					...
					<div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
						<div className="w-full p-4 sm:p-12.5 xl:p-17.5">
							...
							>
								...
								<div className="mb-4">
									<label className="mb-2.5 block font-medium text-black dark:text-white">
										You will receive an email with an unique code for password reset
									</label>
								</div>

								<button
									onClick={send2FAemailHandle}
									className="w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 text-white transition hover:bg-opacity-90">
									Request reset code
								</button>


								{isVisibleResetPasswordInputs &&
									<div className="mb-4 mt-8">
										<div className="relative">
											...
											<button
												...
												className="w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 mt-4 text-white transition hover:bg-opacity-90">
												Reset password
											</button>
										</div>
									</div>
