...
export async function GET(req: NextRequest, { params }: { params: { timestamp: string } }) {
	try {
		...
		const [response] = await pool.execute("SELECT * FROM CompanyInfo");

		if (!Array.isArray(response)) {
			return NextResponse.json({ message: "Error fetching company info", status: 404 }, { status: 404 });
		}
		return NextResponse.json({ response, status: 200 }, { status: 200 });
	} catch (error) {
		return NextResponse.json({ message: "Error fetching company info", status: 500 }, { status: 500 });
	}
}
