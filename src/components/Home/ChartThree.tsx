...
const ChartThree: React.FC = () => {
	...
	const calculatePercentage = (index: number) => {
		...
		setCompaniesEmail((Math.round((Number(totals[index]?.CompanyEmail || 0) * 100) / (total || 1))).toString() + "%");
	};
			...
			<div className="mb-3 justify-between gap-4 sm:flex">
				<div>
					<h5 className="text-xl font-semibold text-black dark:text-white">
						Total SMS & Email Sent
					</h5>
				</div>
				<div>
					<div className="relative z-20 inline-block">
						<select
							onChange={(e) => calculatePercentage(Number(e.target.value))}
							className="relative z-20 inline-flex appearance-none bg-transparent py-1 pl-3 pr-8 text-sm font-medium outline-none"
						>
							<option className="dark:bg-boxdark" value="0">
								Weekly
							</option>
							<option className="dark:bg-boxdark" value="1">
								Monthly
							</option>
							<option className="dark:bg-boxdark" value="2">
								Yearly
							</option>
						</select>
					...
					<div className="w-full px-8 sm:w-1/2">
						<div className="flex w-full items-center">
							...
							<p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
								<span> Clients SMS </span>
								<span id="noClientsSms"> {clientsSMS} </span>
							</p>
						</div>
					</div>
					<div className="w-full px-8 sm:w-1/2">
						<div className="flex w-full items-center">
							...
							<p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
								...
								<span id="noClientsEmail"> {companiesEmail} </span>
							</p>
						</div>
					</div>
					<div className="w-full px-8 sm:w-1/2">
						<div className="flex w-full items-center">
							...
							<p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
								...
								<span id="noCompaniesEmails"> {clientsEmail} </span>
							</p>
						</div>
					</div>
					<div className="w-full px-8 sm:w-1/2">
